...
describe 'Ruby intro part 3' do
  describe 'BookInStock' do
    ...
    describe 'getters and setters' do
      ...
      it 'should set ISBN [1 point]', points: 1 do
        expect(@book.isbn).to eq('isbn1')
      end
      it 'should set price [1 point]', points: 1 do
        expect(@book.price).to eq(33.8)
      end
      it 'should be able to change ISBN [1 point]', points: 1 do
        @book.isbn = 'isbn2'
        expect(@book.isbn).to eq('isbn2')
      end
      it 'should be able to change price [1 point]', points: 1 do
        @book.price = 300.0
        expect(@book.price).to eq(300.0)
      end
    end
    describe 'constructor' do
      it 'should reject invalid ISBN number [1 point]', points: 1 do
        expect { BookInStock.new('', 25.00) }.to raise_error(ArgumentError)
      end
      it 'should reject zero price [1 point]', points: 1 do
        expect { BookInStock.new('isbn1', 0) }.to raise_error(ArgumentError)
      end
      it 'should reject negative price [1 point]', points: 1 do
    ...
    describe '#price_as_string' do
      ...
      it 'should display 33.95 as "$33.95" [1 point]', points: 1 do
        expect(BookInStock.new('isbn11', 33.95).price_as_string).to eq('$33.95')
      end
      it 'should display 1.1 as $1.10 [1 point]', points: 1 do
        expect(BookInStock.new('isbn11', 1.1).price_as_string).to eq('$1.10')
      end
      it 'should display 20 as $20.00 [1 point]', points: 1 do
